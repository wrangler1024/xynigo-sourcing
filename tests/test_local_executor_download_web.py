...
class LocalExecutorDownloadWebTests(unittest.TestCase):
    ...
    def test_runtime_bar_hub_status_uses_selected_executor_heartbeat(self):
        ...
        self.assertIn("pill.textContent = 'HubStudio 已启动'", self.html)
        self.assertNotIn('🟢 HubStudio 已连接', self.html)
        self.assertIn("pill.textContent = 'HubStudio 未启动'", self.html)
        self.assertIn("pill.textContent = 'HubStudio 状态未知'", self.html)
        self.assertIn("selected.hubStatus === 'limited'", self.html)
