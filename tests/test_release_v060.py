# -*- coding: utf-8 -*-
"""Release contract tests for Xynigo Sourcing v0.17.21 candidate."""
...
class ReleaseV0170Tests(unittest.TestCase):
    def test_version_and_packaging_are_aligned(self):
        ...
        self.assertEqual(__version__, '0.17.22')
        pyproject = (root / 'pyproject.toml').read_text(encoding='utf-8')
        self.assertIn('version = "0.17.22"', pyproject)
        cloud_project = (root / 'cloud' / 'auth-service' /
                         'pyproject.toml').read_text(encoding='utf-8')
        self.assertIn('version = "0.17.21"', cloud_project)
        cloud_init = (root / 'cloud' / 'auth-service' / 'src' /
                      'xynigo_auth' / '__init__.py').read_text(
                          encoding='utf-8')
        self.assertIn('__version__ = "0.17.21"', cloud_init)
        cloud_main = (root / 'cloud' / 'auth-service' / 'src' /
                      'xynigo_auth' / 'main.py').read_text(encoding='utf-8')
        self.assertIn('version="0.17.21"', cloud_main)
        self.assertTrue((root / 'release' / 'v0.17.22.zh-CN.json').is_file())
        self.assertTrue((root / 'release' / 'v0.17.22.zh-CN.md').is_file())
        self.assertTrue((root / 'release' / 'v0.17.22.zh-CN.json').is_file())
        self.assertTrue((root / 'release' / 'v0.17.22.zh-CN.md').is_file())
    ...
    def test_release_asset_builder_supports_test_channel(self):
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            asset.write_bytes(b'xynigo-v0.17.21-test-package')
            ...
            self.assertEqual(payload['version'], '0.17.21')
    ...
    def test_rebuilding_sole_platform_refreshes_legacy_manifest_hash(self):
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            command = [
                ...
                '--platform', 'macos-arm64',
                '--asset', str(asset),
                '--notes', str(root / 'release' / 'v0.17.22.zh-CN.json'),
                '--manifest', str(manifest),
                '--sha-file', str(sha_file),
            ]
    ...
    def test_web_bundle_contains_module_three_and_template(self):
        ...
        self.assertIn('Xynigo Sourcing v0.17.21', html)
        self.assertIn('累计修复测试 v0.17.21', html)
        self.assertIn("headers.set('X-Xynigo-Client-Version', '0.17.21-cloud-web')", html)
        ...
        self.assertNotIn('id="sideHelp"', html)
        self.assertIn('id="runtimeStatusBar"', html)
        self.assertIn('id="runtimeDrawer"', html)
        ...
        self.assertIn('id="envVerifySummary"', html)
        self.assertIn('如需调整数量，请在执行器桌面客户端设置中修改', html)
        self.assertNotIn('id="envVerify"', html)
        ...
        self.assertIn(
            ...
            'data-cloud-only="1" data-required-role="super_admin"', html)
        self.assertIn('>云端服务配置<span class="secondary-status restricted">超管</span>', html)
        self.assertNotIn('data-planned="云端服务配置"', html)
        ...
        self.assertIn("let openFeatureTabs = []", html)
        ...
        self.assertIn('飞书企业应用凭证按组织保存于云端并加密落库', html)
        ...
        self.assertIn("button.textContent = '验证并保存中…'", html)
        self.assertIn("setLarkSaveFeedback('bad', '⚠ 保存失败：'", html)
        self.assertIn('await loadLarkConfigStatus()', html)
        self.assertIn('Secret 已在云端加密保存，页面不会回显', html)
        self.assertIn('Secret 永不回显或下发执行器', html)
        self.assertNotIn('id="cfgLarkLedgerUrl"', html)
        self.assertNotIn('id="cfgLarkClearCredential"', html)
        self.assertNotIn('id="btnLarkPreflight"', html)
        self.assertNotIn('id="cfgLarkBaseToken"', html)
        self.assertNotIn('id="cfgLarkTableId"', html)
        self.assertIn('body:JSON.stringify({appId, appSecret, expectedRevision:Number(larkConfigRevision || 0)})', html)
        self.assertIn('/api/lark/config', html)
        self.assertIn("'/v1/admin/integrations/feishu'", html)
        ...
        self.assertNotIn('larkTargetBaseName = targetBaseName', html)
