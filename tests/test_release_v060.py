# -*- coding: utf-8 -*-
"""Release contract tests for Xynigo Sourcing v0.13.9 candidate."""
...
class ReleaseV0139Tests(unittest.TestCase):
    def test_version_and_packaging_are_aligned(self):
        root = Path(__file__).resolve().parents[1]
        self.assertEqual(__version__, '0.13.9')
        pyproject = (root / 'pyproject.toml').read_text(encoding='utf-8')
        self.assertIn('version = "0.13.9"', pyproject)
        cloud_project = (root / 'cloud' / 'auth-service' /
                         'pyproject.toml').read_text(encoding='utf-8')
        self.assertIn('version = "0.13.9"', cloud_project)
        cloud_init = (root / 'cloud' / 'auth-service' / 'src' /
                      'xynigo_auth' / '__init__.py').read_text(
                          encoding='utf-8')
        self.assertIn('__version__ = "0.13.9"', cloud_init)
        cloud_main = (root / 'cloud' / 'auth-service' / 'src' /
                      'xynigo_auth' / 'main.py').read_text(encoding='utf-8')
        self.assertIn('version="0.13.9"', cloud_main)
        self.assertTrue((root / 'release' / 'v0.13.9.zh-CN.json').is_file())
        self.assertTrue((root / 'release' / 'v0.13.9.zh-CN.md').is_file())
    ...
    def test_release_asset_builder_supports_test_channel(self):
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            asset.write_bytes(b'xynigo-v0.13.9-test-package')
            ...
            self.assertEqual(payload['version'], '0.13.9')
    ...
    def test_rebuilding_sole_platform_refreshes_legacy_manifest_hash(self):
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            command = [
                ...
                '--platform', 'macos-arm64',
                '--asset', str(asset),
                '--notes', str(root / 'release' / 'v0.13.9.zh-CN.json'),
                '--manifest', str(manifest),
                '--sha-file', str(sha_file),
            ]
    ...
    def test_web_bundle_contains_module_three_and_template(self):
        ...
        self.assertIn('Xynigo Sourcing v0.13.9', html)
        self.assertIn('累计修复测试 v0.13.9', html)
