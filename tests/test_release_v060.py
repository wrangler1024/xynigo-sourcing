# -*- coding: utf-8 -*-
"""Release contract tests for Xynigo Sourcing v0.12.0 test candidate."""
...
class ReleaseV0120Tests(unittest.TestCase):
    def test_version_and_packaging_are_aligned(self):
        ...
        self.assertIn('version = "0.12.0"', cloud_project)
        cloud_init = (root / 'cloud' / 'auth-service' / 'src' /
                      'xynigo_auth' / '__init__.py').read_text(
                          encoding='utf-8')
        self.assertIn('__version__ = "0.12.0"', cloud_init)
        cloud_main = (root / 'cloud' / 'auth-service' / 'src' /
                      'xynigo_auth' / 'main.py').read_text(encoding='utf-8')
        self.assertIn('version="0.12.0"', cloud_main)
        self.assertTrue((root / 'release' / 'v0.12.0.zh-CN.json').is_file())
        self.assertTrue((root / 'release' / 'v0.12.0.zh-CN.md').is_file())
    ...
    def test_release_asset_builder_supports_test_channel(self):
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            asset.write_bytes(b'xynigo-v0.12.0-test-package')
            ...
            self.assertEqual(payload['version'], '0.12.0')
    ...
    def test_web_bundle_contains_module_three_and_template(self):
        ...
        self.assertIn('Xynigo Sourcing v0.12.0', html)
        self.assertIn('测试候选 v0.12.0', html)
