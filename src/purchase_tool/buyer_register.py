# -*- coding: utf-8 -*-
"""SHEIN 墨西哥买家号注册状态机与 HubStudio 编排。"""
...
class SheinRegistrationFlow(object):
    ...
    def __init__(self, outlook_reader=None, accept_terms=False,
                 ...
                 state_timeout=35, sleep=time.sleep):
    ...
    def _wait_state(self, page, timeout=None):
        ...
        while time.time() < deadline:
            ...
            if SECURITY_FAILED_TEXT in shadow_text:
                return 'security_verification_failed'
            if (SECURITY_EMAIL_VERIFY_TEXT in shadow_text and
                    SECURITY_CODE_LABEL in shadow_text):
                return 'security_email_verification'
            if PHONE_VERIFY_TEXT in text:
    ...
    def _submit_security_code(self, page, code):
        ...
        if hasattr(page, 'click_shadow_text'):
            page.click_shadow_text(SECURITY_SUBMIT_TEXT, 'gee-captcha')
        else:
            page.click_text(SECURITY_SUBMIT_TEXT)
        state = self._wait_state(page)
    ...
    def run(self, page, task):
        ...
        for attempt in range(1, self.max_submit_attempts + 1):
            ...
            page.click_text('Regístrate', exact=False)
            state = self._wait_state(page)
...
class RegistrationOrchestrator(object):
    ...
    def __init__(self, hub, accept_terms=False,
        ...
        self.coupon_inspector = coupon_inspector or CouponInspector()

    @staticmethod
    def env_create_body(name):
        return {
            'containerName': name,
            'tagName': REGISTER_TAG,
            'asDynamicType': 0,
    ...
    def _resolve_env(self, task):
        if task.env_serial:
            env = self.hub.env_by_serial(task.env_serial, REGISTER_TAG)
            if not env:
                raise RegistrationError('未找到环境序号 %s' % task.env_serial)
            return env

        existing = {e.get('containerName'): e
                    for e in self.hub.env_list(REGISTER_TAG)}
        if task.env_name in existing:
            return existing[task.env_name]
        if not PROXY_LINK:
            raise RegistrationError(
                '新建环境前必须设置 XYNIGO_PROXY_LINK')
        self.hub.env_create(self.env_create_body(task.env_name))
        deadline = time.time() + 25
        while time.time() < deadline:
            for env in self.hub.env_list(REGISTER_TAG):
    ...
    def run_one(self, task):
        ...
        try:
            ...
            post_errors = []
            try:
                coupon_note = self.coupon_inspector.inspect(
                    page).remark_fragment()
            except Exception as exc:
                coupon_note = '优惠券:验券失败'
                post_errors.append('验券失败：%s' % scrub_text(exc))

            self.hub.container_add_account(
                code, task.email, task.shein_password, site='MX')
            self.hub.env_update(
