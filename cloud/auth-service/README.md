...
服务器首次从 PyPI 构建镜像可能很慢；当前镜像层已缓存，后续仅在依赖清单变化时重新下载。测试实例当前仍需把云端 App ID/App Secret 从临时使用的 `samforo-cli` 切换为“小犀代采”；测试实例不代表已经生产上线，真实 OAuth 闭环也必须以飞书回调成功、会话落库和退出撤销全部通过为准。
