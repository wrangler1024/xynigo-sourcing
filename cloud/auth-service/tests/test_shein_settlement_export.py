...
def test_export_money_cells_are_numbers_not_text():
    ...
    sheet, _, _ = _sheet(_summary([
        _store("甲店", "MXN", in_transit=Decimal("1234.56"),
               ...
               batches=(PayoutBatch(date(2026, 9, 21), "MXN", Decimal("500.25")),)),
    ]))
...
def test_schedule_block_lists_each_pay_date_with_native_amounts():
    ...
    sheet, _, _ = _sheet(_summary([
        _store("甲店", "MXN", batches=(
            PayoutBatch(date(2026, 9, 21), "MXN", Decimal("600.00")),
            PayoutBatch(date(2026, 9, 28), "MXN", Decimal("400.00")),
        )),
    ]))
    rows = [[c.value for c in row] for row in sheet.iter_rows()]
    schedule = [r for r in rows if r[2] in ("2026-09-21", "2026-09-28")
                and r[4] and "MXN:" in str(r[4])]
    assert [r[2] for r in schedule] == ["2026-09-21", "2026-09-28"]
...
def test_pay_dates_column_lists_all_batches():
    ...
    sheet, _, _ = _sheet(_summary([
        _store("甲店", "MXN", batches=(
            PayoutBatch(date(2026, 9, 28), "MXN", Decimal("1.00")),
            PayoutBatch(date(2026, 9, 21), "MXN", Decimal("2.00")),
        )),
    ]))
    assert sheet[2][6].value == "2026-09-21 / 2026-09-28"
