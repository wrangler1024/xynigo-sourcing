...
- **对账单**（`get-check-order-list`）：窗口按"对账单生成时间"算且必须
  **≤7 天整**，越界是**硬报错**（gsfs99401），不是自动截断。所以分片必须
  自己算准，且边界要清零微秒——带毫秒会让"恰好 7 天"变成 7 天零几毫秒而报错。
- **订单列表**（`order-list`）：窗口 ≤48h。它的问题不是报错而是**漏单**：
  一张 5 天前发货、至今未签收的订单，若 48h 内没有更新时间就不会出现在
  窗口里 → 直接查会把在途算少。故必须按 48h 分片向前回溯建本地台账，
  之后只拉增量。
...
# 窗口上限（实测口径，见 shein_openapi_client 常量说明）
CHECK_ORDER_MAX_SPAN = timedelta(days=7)
ORDER_MAX_SPAN = timedelta(hours=48)
