...
当前线上协同测试版为 `v0.12.4`，GitHub Release 提供 Windows x86_64 与 macOS ARM64 在线更新包。项目正在持续开发，尚未提供公开生产 SaaS；现有授权测试成员可直接访问共享测试 Web 工作台。

`v0.12.4` 在 v0.12.3 云端工作台基础上，将采购协作导入迁移到云端，并把桌面默认入口切换为云端工作台；本地执行器仍保留 HubStudio/CDP/SHEIN 能力及独立兼容入口。该版本面向 `https://xynigo.samforo.icu` 共享测试环境的现有测试用户，不代表生产开放。
