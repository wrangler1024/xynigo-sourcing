...
当前协同测试候选版为 `v0.12.0`：升级采购任务公共认领池和采购员个人执行工作台，补充批量认领、店铺/运营筛选、利润摘要、快捷下单与安全退回入口，并降低并发创建 HubStudio 环境时触发请求频率限制的概率。该版本面向 `https://xynigo.samforo.icu` 共享测试环境的全体现有测试用户，不代表生产开放。
