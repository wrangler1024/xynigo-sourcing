...
当前稳定版为 `v0.10.0`。项目正在持续开发，尚未提供公开生产 SaaS；现有授权测试成员可直接访问共享测试 Web 工作台。

当前协同测试候选版为 `v0.12.3`：在 v0.12.1 日用结果双写和 v0.12.2 数据库事实源基础上，将完整买家号、密码、Cookie、接码资料和业务字段加密写入 PostgreSQL；资源中心以额外凭证读取权限查询完整数据，独立测试 Base 持续镜像 41 个字段，并停用测试版对旧统一台账的运行时读写。该版本面向 `https://xynigo.samforo.icu` 共享测试环境的现有测试用户，不代表生产开放。
