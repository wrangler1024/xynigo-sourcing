...
## 动态操作进度放哪里：一条常驻的「运行状态条」，钉在类型卡下方

![运行状态条](./20260915-procurement-after-sale-v2-runstrip.png)

本模块有**三段会跑的操作**：扫描可申请订单、批量提交售后、刷新退款进度。进度只应有一个归属地，否则用户要在几个卡片之间找。

### 结论

在**类型选择卡的紧下方、① 选择范围之上**放一条运行状态条，三段操作共用：

```
① 售后类型  [丢件退款] [催促发货·规划] [取消订单·规划]
━ 运行状态条（sticky，压在工作台顶栏下方） ━━━━━━━━━━━━━━━━━━━━━━━━
  ◐ 扫描运行中                              ████████░░░░  已扫描 3/6 个环境  [停止]
    共 6 个环境 · 只读，不会改动订单
② 可申请清单  ③ 提交结果  ④ 退款跟踪
```
...
1. **位置固定**：三段操作共用一条，用户永远只看一个地方。原先「进度条在①、阶段文案在③」是分裂的，提交的进度会显示在①（那里的进度条是扫码阶段用的），逻辑上错位。
2. **在动作与结果之间**：扫描按钮在它下面一行（点完抬头就是进度）；结果表在它下面往下长，进度不会被表格推走。
3. **sticky 常驻**：长清单滚动时（比如扫了 300 个环境、或提交几十单）进度条不会滚出视野。工作台已有 8 处 `position: sticky` 先例（顶栏、表头、侧栏、采购执行的操作列等），不是新发明。
4. **与工作台惯例一致**：物流查询、店铺结算查询都是「阶段横幅 + 进度条 + 停止」并排放在结果之上，本模块沿用 `.query-phase-banner` 与既有进度条组件，没有新造。

### 宏观与微观的分工

- **状态条 = 宏观**：整体 N/M、当前阶段、预计剩余、停止。
- **表格行 = 微观**：每一单/每个环境自己的状态（等待 / 进行中 / 已完成 / 失败），已经在三张表的「状态」列里。

两者职责不重叠，所以状态条里不再重复列单条信息，只留「当前正在处理谁」一句。

### 三段操作各自的文案

| 阶段 | 标题 | 说明 | 右侧 |
|---|---|---|---|
| 扫描中 | 扫描运行中 | 共 N 个环境 · 只读，不会改动订单 | 进度条 + `已处理 x / N 个环境 ｜ 平均 — ｜ 预计还需 …` + 停止 |
| 提交中 | 提交售后中 | 退款路径原路退回 · 逐单串行，单间随机停顿 | 进度条 + `已处理 x / N 单 ｜ 平均 22秒/单 ｜ 预计还需 1 分 20 秒` + 停止 |
| 回访中 | 正在回访退款进度 | 只回访未终态的单，到终态即冻结 | 进度条 + 已回访 x/N 单 + 停止 |
| 扫描完成 | 扫描完成 | 可申请 N 单，勾选后点「提交售后」 | 100% + 收起 |
| 提交完成 | 提交完成（部分失败） | 已受理 N 单，退款审核中（结果 7 天内出） | 100% + 收起 |
| 正在停止 | 正在停止 | 在途订单完成后终止 | 保持 spinner |

终态后**自动收起**（默认延迟 2.2 秒收起，先让用户看到完成文案），折成一行只留标题与「展开」；点展开还原完整形态。长会话下不占纵向空间。

![终态已收起](./20260915-procurement-after-sale-v2-strip-collapsed.png)

### 实现细节（已在这个样稿里验证）

状态条不是重新搭的：把真实页面里既有的 `#asPhaseBanner`（阶段横幅）连同 ① 里的进度条、进度文本、停止按钮**整块搬进**同一条，因此 `asSetPhase()` / `asProgress()` 写的还是同一批节点，没有重新接线。滚动容器是工作台的 `<main>`（`overflow:auto`，顶栏不在其中），所以 sticky 用 `top: 8px` 即可紧贴其顶部，视觉上正好落在工作台顶栏下方。

### 收起态的两处实现坑（都已踩过并修正）

1. **表头复选框的 id 不能丢**。样稿按类型重写 ② 表头时，一度把复选框的 `id="asChkHead"` 漏掉，而真实页面的 `asSyncSelection()` 要写这个节点——结果是**每次轮询都抛 `Cannot set properties of null`**，表现为「扫描一直完不成、连续 5 次报状态刷新失败」。重写既有表头时，必须保留原节点上的 id 与契约属性。
2. **不要给进度区写内联 `display`**。收起靠 `display:none`，而内联样式优先级高于样式表，内联写了 `display:flex` 就永远收不起来。排版改用 class（`.as-strip-progress`）。

### 进度标签与「预计还需」：直接复用物流查询那套

物流查询的进度标签是 `已处理 x / N ｜ 平均 … ｜ 本轮重查 … ｜ 累计执行 00:00`，店铺结算查询当初就是照它抄的。售后这次**沿用同一套形态**，但按售后口径裁剪：

| 段 | 是否保留 | 说明 |
|---|---|---|
| 已处理 x / N | 保留 | 单位随阶段变：扫描论「个环境」，提交与回访论「单」 |
| 平均 …/单位 | 保留 | 取各行 `durationSeconds` 的均值 |
| 本轮重查 | **去掉** | 售后没有补采概念；将来若做「补提失败」再加回来 |
| 累计执行 00:00 | **去掉** | 售后没有跨批次累计耗时；本轮耗时已由状态条体现 |
| **预计还需** | **新增** | 售后每单要开浏览器走完整流程（20~40 秒/单），操作员要知道「要不要等着」 |

**采样窗口必须调**：店铺结算的 ETA 用「5 秒内完成 ≥2 个」就出估计，但售后单耗时长，这个窗口几乎凑不满、会一直显示不出估计。改成「**至少完成 1 个且窗口 ≥15 秒**」——按 20~40 秒/单的节奏，第一单跑完就已经越过 15 秒，估计立刻可用。

复用的是**形态与算法**，不是现成函数：`elapsedClock()` 这类全局帮助函数可以直接用，但那段标签逻辑目前在物流查询与店铺结算里**各写了一份**（两处近乎相同的字符串拼接）。售后接上后就是第三份——建议后续抽一个共享函数（入参：已完成/总数/单位/各行耗时/是否显示重查与累计），三个模块改调它。本次先不动既有模块，避免顺手改出回归。
