...
## 三级菜单形态：模块内分段控件（Jeff 拍板）

不改二级页签结构（导入分单 / 采购任务 / 采购执行 / 采购售后 / 异常协同 保持不变），类型做成模块 ① 里的 chip 分段控件，切换即换清单列、结果列与动作文案。页签栏不膨胀；一次只做一种类型，认知负担最低。

样稿里可直接体验到的差异：

- **丢件退款**：① 三段式原样；提交按钮「提交售后」；提交前无二次确认。
- **催发货**：清单多「已催次数 / 下次可催」两列；按钮变「批量催发货」；「冷却中」的行不可勾选。
- **取消订单**：清单多「可否取消」列；**提交前弹出二次确认**——列出将取消的单、合计金额、不可逆提示，并要求输入「取消订单」四个字才解锁确认按钮。

顶部横幅已注明「规划类型，本次未实现」，chip 上也带「规划」字样，避免被当成已实现能力。

## 四条必须一起定的规则

1. **权限要按类型拆。** 现在整模块共用 `assistant.access`——加上「取消订单」后，有采购权限的人就能批量取消订单。应拆成 `procurement.aftersale.refund` / `.urge` / `.cancel`，高危类型单独授权。样稿的类型提示里已把这个权限码显示出来。
...
与主原型一致——逐字复制真实工作台页面，注入数据 mock 后，再叠一层设计补丁（`build_procurement_after_sale_multitype_prototype.py`）。补丁只做三件事：把 ① 的类型胶囊换成 chip 分段控件、按类型切换两张表的列与说明、给高危类型演示二次确认弹层。所有元素复用工作台既有 class（`.chip` / `.modal-mask` / `.modal` / `.pill` / `.card`），不引入新配色。数据全部为合成数据。

## 与当前实现的关系

- 当前代码只实现「丢件退款」，类型是常量 `AFTER_SALE_TYPE`，页面上是一个只读胶囊（见 `20260915-procurement-after-sale-v1.md`）。
- 本样稿是把那个胶囊升级为分段控件之后的形态预览；真正落地时按第 4 条的配置表改造，并同步拆权限。
