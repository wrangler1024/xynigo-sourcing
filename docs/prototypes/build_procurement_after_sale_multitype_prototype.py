...
(function () {
  ...
  // 阶段迷你时间轴：四节点固定顺序，当前节点高亮；已拒绝把末节点标红
  const TL_ORDER = ['submitted', 'reviewing', 'processing', 'refunded'];
  const TL_LABEL = { submitted: '已受理', reviewing: '审核中', processing: '处理中',
                     refunded: '已退款', rejected: '已拒绝', overdue: '超期未出结果' };
  function timelineCell(phase) {
    const rejected = phase === 'rejected';
    const idx = rejected ? TL_ORDER.length - 1 : Math.max(0, TL_ORDER.indexOf(phase));
    let html = '<span class="as-tl">';
    for (let i = 0; i < TL_ORDER.length; i++) {
      if (i) html += `<u class="${i <= idx ? 'done' : ''}"></u>`;
      const cls = rejected && i === idx ? 'bad' : (i < idx ? 'done' : (i === idx ? 'cur' : ''));
      html += `<i class="${cls}"></i>`;
    }
    html += '</span>';
    return `<td>${html}<span class="as-tl-label">${esc(TL_LABEL[phase] || phase)}</span></td>`;
  }
  ...
  function renderTracking() {
    ...
    body.innerHTML = TRACKING.map(r => {
      ...
      return `<tr${bad ? ' class="tr-bad"' : (r.phase === 'refunded' ? '' : ' class="tr-run"')}>`
        ...
        + `<td>${esc(r.result)}</td>`
        + timelineCell(r.phase)   // 该函数自带 <td>，外层不要再包一层
        + `<td>${esc(r.left)}</td>`
  ...
  function installTracking() {
    ...
    card.innerHTML = `<div class="card-title" style="padding:17px 19px 0;">
        ...
        <div class="table-actions">
          <button class="btn sm" id="asTrackRefresh">刷新退款进度</button>
          <div class="export-split">
            <button class="btn primary" id="asTrackExport">导出 Excel</button>
            <button class="btn primary" id="asTrackExportMenuBtn">▾</button>
            <div class="export-menu" id="asTrackExportMenu" hidden>
              <button id="asTrackExportStd"><b>标准导出</b><small>退款单号/订单号/环境/金额/阶段/倒计时/退款账户/最近检查，可直接发财务对账</small></button>
              <button id="asTrackExportFull"><b>完整导出</b><small>在标准列基础上加平台时间轴原文与处置建议，便于排查</small></button>
              <button id="asTrackExportOpen"><b>待跟进导出</b><small>仅未终态（已受理/审核中/处理中）与异常（已拒绝/超期）的行</small></button>
            </div>
          </div>
        </div>
      </div>
      ...
      <div style="padding: 0 19px;">
        <div class="table-scroll scroll-20">
          <table><thead><tr>
            <th>环境序号</th><th>订单号</th><th style="width:46px">商品图</th><th>退款单号</th>
            <th>退款信用卡</th><th>退款金额</th><th>阶段</th><th>剩余倒计时</th>
            <th>最近检查</th><th>处置</th>
    ...
    const exportMenu = document.getElementById('asTrackExportMenu');
    const openExport = () => { exportMenu.hidden = !exportMenu.hidden; };
    document.getElementById('asTrackExport').onclick = openExport;
    document.getElementById('asTrackExportMenuBtn').onclick = e => { e.stopPropagation(); openExport(); };
    document.addEventListener('click', () => { exportMenu.hidden = true; }, true);
    const exportDemo = variant => {
      exportMenu.hidden = true;
      const rows = variant === 'open'
        ? TRACKING.filter(r => !['refunded', 'rejected'].includes(r.phase) || r.phase === 'rejected')
        : TRACKING;
      toast(`样稿演示：正式版导出「${variant}」共 ${rows.length} 行（服务端生成 xlsx/csv）`);
    };
    document.getElementById('asTrackExportStd').onclick = () => exportDemo('standard');
    document.getElementById('asTrackExportFull').onclick = () => exportDemo('full');
    document.getElementById('asTrackExportOpen').onclick = () => exportDemo('open');
